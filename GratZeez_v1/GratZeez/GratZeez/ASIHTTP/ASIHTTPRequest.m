...
- (void)attemptToApplyCredentialsAndResume
{
	...
	if (!CFHTTPAuthenticationRequiresAccountDomain(requestAuthentication)) {
		...
		if ([self authenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM || [self authenticationRetryCount] == 0) {
			...
			ASI_DEBUG_LOG(@"[AUTH] Request %@ NTLM gratzeez step %i",self,[self authenticationRetryCount]+1);
		}
	#endif
...
+ (void)showNetworkActivityIndicator
{
    ...
    if (!hud)hud = hud = [[MBProgressHUD alloc] initWithView:frontWindow];
	[frontWindow addSubview:hud];
	[hud show:YES];
